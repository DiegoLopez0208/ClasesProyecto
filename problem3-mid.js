...
const movies = [
  ...
  function listUsersWithMovies(users, movies) {
    let dateUserandMovie  = users.map(function(usuario) {
      let movieSaw = movies.filter(function(pelicula) {
        return pelicula.userId === usuario.id;
      });
      let calificationPromedy = movieSaw.reduce(function(total, pelicula) {
        return total + pelicula.rate;
      }, 0) / movieSaw.length;
      return {
        name: usuario.name,
        email: usuario.email,
        ...
        movieSaw: movieSaw.map(function(pelicula) {
          return pelicula.movie;
        }),
        calificationPromedy: califitacionPromedy
      };
    });
    return dateUserandMovie;
  }
  let dateUserandMovie = listUsersWithMovies(users, movies);
  console.log(dateUserandMovie);